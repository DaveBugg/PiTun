    ...
    content: {
      en: (
        <>
          ...
          <P><B>From the panel:</B> the Update button writes a request that a host-side agent carries out — it is installed by the installer, or by hand with <code className="text-gray-400">scripts/pitun-update.sh --install-agent</code>. Without it the button waits forever.</P>
          <P><B>Unattended:</B> <code className="text-gray-400">scripts/pitun-update.sh --install-timer</code> adds a daily systemd timer that reports by default and only applies with <code className="text-gray-400">--apply</code>. That timer does NOT service the panel's button — it is a separate, scheduled check.</P>
        </>
      ),
      ru: (
        <>
          ...
          <P><B>Из панели:</B> кнопка обновления пишет запрос, который выполняет агент на хосте — его ставит установщик либо вручную <code className="text-gray-400">scripts/pitun-update.sh --install-agent</code>. Без него кнопка будет ждать бесконечно.</P>
          <P><B>Без участия:</B> <code className="text-gray-400">scripts/pitun-update.sh --install-timer</code> добавляет ежедневный systemd-таймер, который по умолчанию только отчитывается и применяет только с <code className="text-gray-400">--apply</code>. Этот таймер кнопку из панели НЕ обслуживает — это отдельная проверка по расписанию.</P>
        </>
      ),
    },
  },
